...
class AdaptiveLearningSystem {
    ...
    recordAttempt(exercise, isCorrect) {
        ...
        if (isCorrect)
            this.performanceData.categories[category].correct++;
        // Track by difficulty
        ...
        if (isCorrect)
            this.performanceData.difficulties[difficulty].correct++;
        // Track by unit
        ...
        if (isCorrect)
            this.performanceData.units[unit].correct++;
    ...
    selectNextExercise(availableExercises, currentDifficulty = 1) {
        ...
        if (reviewExercises.length > 0) {
            ...
            const reviewable = availableExercises.filter(ex => reviewIds.includes(ex.id || `${ex.type}-${ex.concept}`));
        ...
        const suitableExercises = availableExercises.filter(ex => (ex.difficulty || 1) === targetDifficulty);
