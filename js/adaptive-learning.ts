/**
 * @fileoverview Adaptive Learning System
 *
 * Features:
...
interface PerformanceData {
  ...
  difficulties: Record<number, DifficultyData>;
...
class AdaptiveLearningSystem {
   ...
   */
  recordAttempt(exercise: Exercise, isCorrect: boolean): void {
    const now = Date.now();
    ...
    if (!this.performanceData.concepts[concept]) {
      this.performanceData.concepts[concept] = {
        ...
        lastSeen: now,
        mastery: 0,
        difficulty: (exercise.difficulty as unknown as number) || 1
      };
    }
    ...
    // Track by category
    const category = (exercise as any).category || 'general';
    ...
    // Track by difficulty
    const difficulty = (exercise.difficulty as unknown as number) || 1;
    ...
    // Track by unit
    const unit = (exercise as any).unitNumber || 1;
   ...
   */
  isConceptMastered(concept: string): boolean {
   ...
   */
  getExercisesForReview(): ReviewExercise[] {
    const now = Date.now();
   ...
   */
  selectNextExercise(availableExercises: Exercise[], currentDifficulty = 1): Exercise | null {
   ...
   */
  createAdaptiveSequence(allExercises: Exercise[], length = 50): Exercise[] {
    ...
    while (sequence.length < length && available.length > 0) {
      ...
      sequence.push(next);
      currentDifficulty = (next.difficulty as unknown as number) || currentDifficulty;
